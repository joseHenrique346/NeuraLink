...
public class FineNodeDTO : BaseDTO
{
    [Required]
    ...
    public FineNodeDTO(string code, string nodeConsultation)
    {
        Code = code;
        NodeConsultation = nodeConsultation;
    }

    public FineNodeDTO(string code, string? node, string? type, double? value, string? article, EnumSeverityType? severity)
    {
        Code = code;
        Node = node;
        Type = type;
